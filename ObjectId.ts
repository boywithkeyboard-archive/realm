...
const ByteUtils = {
  ...
  fromHex(hex: string): Uint8Array {
    ...
    for (let i = 0; i < evenLengthHex.length; i += 2) {
      ...
      if (!hexDigit.test(firstDigit)) {
        break
      }

      if (!hexDigit.test(secondDigit)) {
        break
      }

      const hex = Number.parseInt(`${firstDigit}${secondDigit}`, 16)
...
abstract class BSONValue {
  ...
  /** @internal */
  get [Symbol.for('@@mdb.bson.version')](): number {
    return 5
  }
